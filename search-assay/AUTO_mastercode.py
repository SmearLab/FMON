...
while True:
    datapath = base_dir + COM_coordinates + "/" + group_name + "/" + str(session_num) + "/"
...
def ni_handler(): #define background function to handle incoming NI data
    while True:
        ...
        signaldata[2,:].tofile(ch0_handle); 
nisignal = threading.Thread(target = ni_handler) #set handler function in background
nisignal.daemon = True

##INITIATE SESSION
print "PID " + str(COM_coordinates) + ", Session " + str(session_num) #report session initiation
